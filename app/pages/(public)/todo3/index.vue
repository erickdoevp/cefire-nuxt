...
<template>
  <div class="flex flex-col items-center">
    <h1 class="mr-4 mb-4">TODO APP 3</h1>
    <form class="form" @submit.prevent="store.addTodo({ name: form.name, color: form.color })">
    ...
    <div v-if="store.todos.value.length > 0" class="list">
      ...
      <div 
        ...
        <div class="list__element-actions">
          ...
          <template v-if="!todo.isEditing">
            <button 
              type="button"
              @click="store.startEdit(todo)"
              :disabled="store.todos.value.some(t => t.isEditing)"
              >
              Edit
            </button>
          ...
          <template v-else>
            <button 
              @click="store.saveEdit(todo)"
              >
              Aceptar
            </button>

            <button
              @click="store.cancelEdit(todo)"
            >
              Cancelar
            </button>
          </template>
